...
def create_workflow(
    ...
    cursor: optional caller-supplied cursor already inside an open
            repo.transaction() block. When provided, this function
            executes on that cursor WITHOUT opening its own
            transaction, so the caller can fold this insert into a
            larger atomic transaction (e.g. Workflow + 
            WorkflowResearchConfig as one unit). When omitted
            (default), behavior is unchanged from before: this
            function opens and commits its own transaction.
    ...
    sql = """
        ...
        VALUES (?, ?, ?, ?, ?);
        """
    params = (name, current_stage, status, timestamp, timestamp)
